...
1) 좌우대칭   그림을 좌우로 뒤집어 원본과 겹쳐 차이를 잰다.
              차이가 없을수록 「정면 박제」. T 포즈·증명사진 구도가 여기 걸린다.
...
def measure(im):
    ...
    # --- 1) 좌우대칭 -----------------------------------------------------
    flip = list(g.transpose(Image.FLIP_LEFT_RIGHT).getdata())
    sym = sum(abs(a - b) for a, b in zip(px, flip)) / (len(px) * 255.0)
...
def judge(asset_id, key, m, prompt):
    ...
    if m["sym"] < 0.045 and not sym_exempt:
        bad.append((
            "정면대칭",
            "좌우가 거의 완전히 겹친다 (%.3f). 카메라를 정면에 놓고 인물을 한가운데 세운 증명사진 구도다." % m["sym"],
            "몸을 반측면으로 틀거나, 한쪽 팔만 올리거나, 카메라를 조금 옆·아래로 옮긴다.",
        ))
    elif m["sym"] < 0.075 and not sym_exempt:
        warn.append(("대칭기울음", "좌우가 꽤 닮았다 (%.3f)." % m["sym"], "한쪽에만 무게를 준다."))
    ...
    if m["sd"] < 0.085 and not night:
        ...
        warn.append(("밤인데도평평", "밤 장면이지만 밝기 폭이 거의 없다 (%.3f)." % m["sd"],
                     "달빛이든 등불이든 광원을 하나 정해 밝은 점을 만든다."))
