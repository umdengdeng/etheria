...
**비치는 천은 알파를 채워야 한다** — 얇은 천이 초록 위에 겹치면 렌더 결과가
「흰색+초록」의 중간색이라 키잉하면 **반투명**으로 남는다. 그대로 두면 게임에서
뒤 배경이 드레스에 얼룩덜룩 비친다. 그래서 실루엣 **안쪽은 무조건 불투명**으로 채우고
부드럽게 깎는 건 바깥 경계 한 겹에서만 한다.

**머리카락 청록 테두리** — 검은 머리에 초록이 섞이면 G 를 깎아도 `(R,B,B)` 꼴이 남아
**청록**으로 뜬다. 이 캐릭터 팔레트엔 초록이 없으니(흰 드레스·검은 머리·붉은 눈·은관)
**G·B 가 R 보다 뜨는 픽셀은 전부 무채색으로 누른다.** 흰 드레스는 R≈G≈B,
피부는 R>G>B 라 안 걸린다.
